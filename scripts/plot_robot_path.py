...
Each line of the input file is "timestampMillis,x,y" (no header), appended once
per SteeringPilot.Movement. This script parses the file and plots the (x, y)
path with matplotlib, using the millisecond timestamps to color the path by
time and to compute the elapsed time between points.

Usage:
    python3 plot_robot_path.py output.txt
    python3 plot_robot_path.py output.txt --save path.png
"""
import argparse
import csv
import os
...
def plot_path(timestamps, xs, ys, title):
    fig, ax = plt.subplots()
...
def main():
    ...
    parser.add_argument("logfile", help="Path to the log file written by BasicMovementListener (e.g. output.txt)")
    ...
    timestamps, xs, ys = parse_log(args.logfile)
    fig = plot_path(timestamps, xs, ys, title=f"Robot path ({os.path.basename(args.logfile)})")
