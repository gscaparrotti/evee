 ...
 *
 */
public class OldSteeringPilot implements ArcMoveController, RegulatedMotorListener {
	 ...
	 */
	public OldSteeringPilot(double driveWheelDiameter, RegulatedMotor driveMotor,
	...
	public void travelArc(double turnRadius, double distance, boolean immediateReturn) throws IllegalArgumentException {
		...
		moveEvent = new Move(true , (float) angle, (float)(distance * Math.toRadians(angle)));
...
//		if(!isMoving()) return false; // Should return no movement? Or moveEvent? Null might be appropriate.
	...
	public void rotationStopped(RegulatedMotor motor, int tachoCount,boolean stall, long timeStamp) {
		...
		moveEvent = new Move(isMoving ,angle, (float)((double)distance * Math.toRadians(angle)));
